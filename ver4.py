...
# Use the same HTML templates as before (they remain unchanged)
INDEX_HTML = '''
<!DOCTYPE html>
<html lang="en">
<head>
...
class BitcoinAIAssistant:
    ...
    def calculate_technical_indicators(self, prices):
        """Calculate comprehensive technical indicators without scikit-learn"""
        if len(prices) < 20:
        ...
        # Volatility
        volatility_5 = np.std(prices[-5:]) / np.mean(prices[-5:]) * 100 if len(prices) >= 5 else 0
        volatility_10 = np.std(prices[-10:]) / np.mean(prices[-10:]) * 100 if len(prices) >= 10 else 0
        volatility_20 = np.std(prices[-20:]) / np.mean(prices[-20:]) * 100 if len(prices) >= 20 else 0
        
        # Momentum
        momentum_1 = ((prices[-1] - prices[-2]) / prices[-2] * 100) if len(prices) >= 2 else 0
        ...
        price_vs_sma_20 = ((prices[-1] - sma_20) / sma_20) * 100
        price_vs_bb_upper = ((prices[-1] - bb_upper) / bb_upper) * 100
        price_vs_bb_lower = ((prices[-1] - bb_lower) / bb_lower) * 100
    ...
    def calculate_rsi(self, prices, period=14):
        ...
        rs = avg_gains[period:] / np.where(avg_losses[period:] == 0, 1, avg_losses[period:])
        rsi = 100 - (100 / (1 + rs))
    ...
    def calculate_macd(self, prices, fast=12, slow=26, signal=9):
        ...
        for i in range(slow, len(prices)):
            fast_ema = ema(prices[i-slow:i] if i>=slow else prices[:i], fast)
            slow_ema = ema(prices[i-slow:i] if i>=slow else prices[:i], slow)
    ...
    def ml_prediction(self, df):
        """Advanced machine learning prediction without scikit-learn"""
        ...
        try:
            ...
            features = [
                ...
                indicators['price_vs_sma_20'] / 100,
                indicators['momentum_5'] / 100,
                indicators['volatility_20'] / 100,
                indicators['macd_histogram'] / prices[-1] if prices[-1] != 0 else 0,
                (indicators['price_vs_bb_upper'] + indicators['price_vs_bb_lower']) / 200,
            ]
            
            # Weighted scoring system (simplified ML)
            ...
            if weighted_score > 0.05:
                direction = 'UP'
                confidence = min(90, 50 + abs(weighted_score) * 800)
            ...
            pred_linear = slope * len(prices) + intercept
            
            # Method 2: Moving average projection
            recent_trend = (prices[-1] - prices[-5]) / 5 if len(prices) >= 5 else 0
            pred_ma = prices[-1] + recent_trend
            
            # Method 3: Volatility-adjusted prediction
            volatility = indicators['volatility_20'] / 100
            pred_vol = prices[-1] * (1 + weighted_score * (1 + volatility))
            
            # Combine predictions
            next_price = (pred_linear * 0.4 + pred_ma * 0.3 + pred_vol * 0.3)
            
            # Ensure positive price
            next_price = max(1000, next_price)
            
            # Calculate trend strength
            trend_strength = abs(weighted_score) * 1000
    ...
    def simple_prediction(self, prices):
        ...
        return {
            ...
            'weighted_score': recent_trend
        }
    
    def get_default_indicators(self):
        """Return default indicators when data is insufficient"""
        return {
            'sma_5': 45000, 'sma_10': 45000, 'sma_20': 45000,
            'rsi': 50, 'bb_upper': 46000, 'bb_middle': 45000, 'bb_lower': 44000,
            'macd_line': 0, 'macd_signal': 0, 'macd_histogram': 0,
            'volatility_5': 2, 'volatility_10': 2, 'volatility_20': 2,
            'momentum_1': 0, 'momentum_5': 0, 'momentum_10': 0,
            'price_vs_sma_20': 0, 'price_vs_bb_upper': 0, 'price_vs_bb_lower': 0
        }
    ...
    def get_ai_analysis(self, df, account_balance=1000, risk_per_trade=2):
        ...
        # Get ML predictions
        ml_prediction = self.ml_prediction(df)
